...
const Utils = (() => {
  function setStatus(msg, isError = false) {
    ...
    if (el) { el.textContent = msg; el.style.color = isError ? 'var(--danger)' : ''; }
  }

  function setDataInfo(info) {
    const el = document.getElementById('dataInfo');
    if (el && info) el.textContent = `${info.rows.toLocaleString()} rows × ${info.cols} cols`;
  ...
  function showSpinner(label = 'Processing…') {
    ...
    if (s) { s.classList.remove('hidden'); if (l) l.textContent = label; }
  }
  ...
  function populateSelect(selectId, columns, addEmpty = true, emptyText = 'Select…') {
    ...
    const cur = el.value;
    el.innerHTML = '';
    if (addEmpty) el.appendChild(new Option(emptyText, ''));
    columns.forEach(c => el.appendChild(new Option(c, c)));
    if (cur && columns.includes(cur)) el.value = cur;
  }
