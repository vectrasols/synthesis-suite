...
const VizTab = (() => {
  ...
  function updateVisualizeAvailability() {
    ...
    if (exportBtn) exportBtn.disabled = !hasPlottedChart();
  }
  ...
  function onDataLoaded(info) {
    ...
    const filterPick = document.getElementById('filterPick');
    if (filterPick) filterPick.innerHTML = '<option value="">Pick value…</option>';
  ...
  async function exportChart() {
    ...
    if (!hasPlottedChart()) return Utils.toast('Plot a chart before exporting', 'error');
    try {
      ...
      if (usingNativeSave && !savePath) return;
